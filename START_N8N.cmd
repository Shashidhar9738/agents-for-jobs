@echo off
rem Delayed expansion is deliberately NOT enabled: it eats '!' in passwords and
rem other values loaded from .env. Blocks that would need it use subroutines.
setlocal
cd /d "%~dp0"

set "WORKSPACE=%~dp0"
if "%WORKSPACE:~-1%"=="\" set "WORKSPACE=%WORKSPACE:~0,-1%"
set "N8N_HOME=%LOCALAPPDATA%\AIJobAgent\n8n"
set "LOGFILE=%WORKSPACE%\setup.log"
set "ENVFILE=%WORKSPACE%\.env"
set "N8N_BIN=%N8N_HOME%\node_modules\.bin\n8n.cmd"
set "N8N_VERSION=1.50.0"

echo.
echo [INFO] ========== n8n Startup Script ==========
echo [INFO] Workspace: %WORKSPACE%
echo [INFO] n8n Home : %N8N_HOME%
echo [INFO] Log File : %LOGFILE%
echo [INFO] ==========================================
echo.

rem --- Load configuration (never hardcode credentials in this file)
if not exist "%ENVFILE%" (
  echo [ERROR] No .env found at %ENVFILE%
  echo [ERROR] Copy .env.example to .env and fill it in, then re-run.
  pause
  exit /b 1
)
call "%WORKSPACE%\load-env.cmd" "%ENVFILE%"
if errorlevel 1 (
  echo [ERROR] Failed to load %ENVFILE%
  pause
  exit /b 1
)
if not defined N8N_PORT set "N8N_PORT=5678"
if not defined N8N_HOST set "N8N_HOST=0.0.0.0"
if not defined N8N_SECURE_COOKIE set "N8N_SECURE_COOKIE=false"

if not exist "%N8N_HOME%" mkdir "%N8N_HOME%"

rem --- Detect a supported Node version (prefer 20, then 22)
set "NODE_EXE="
call :find_winget_node "OpenJS.NodeJS.20_*"
call :find_winget_node "OpenJS.NodeJS.22_*"

if not defined NODE_EXE call :check_system_node

if not defined NODE_EXE (
  echo.
  echo [ERROR] No supported Node.js found. System has: %SYS_NODE% ^(need 18, 20, or 22^)
  echo.
  echo [FIX] Install Node 22:
  echo   winget install --id OpenJS.NodeJS.22 -e --scope user --accept-package-agreements --accept-source-agreements
  echo.
  pause
  exit /b 1
)

echo [INFO] Using node: %NODE_EXE%
"%NODE_EXE%" -v

for %%I in ("%NODE_EXE%") do set "NODE_DIR=%%~dpI"
set "PATH=%NODE_DIR%;%PATH%"

set "NPM_CMD=%NODE_DIR%npm.cmd"
if not exist "%NPM_CMD%" set "NPM_CMD=npm"

set "SYSTEM_CMD=%SystemRoot%\System32\cmd.exe"
if not exist "%SYSTEM_CMD%" set "SYSTEM_CMD=C:\Windows\System32\cmd.exe"
set "ComSpec=%SYSTEM_CMD%"
set "npm_config_script_shell=%SYSTEM_CMD%"

if not exist "%N8N_HOME%\package.json" (
  echo [INFO] Initializing n8n folder...
  pushd "%N8N_HOME%"
  "%NODE_EXE%" -e "require('fs').writeFileSync('package.json', JSON.stringify({name:'n8n',version:'1.0.0',private:true}, null, 2))"
  popd
)

set "NEEDS_INSTALL=0"
if not exist "%N8N_BIN%" set "NEEDS_INSTALL=1"
if not exist "%N8N_HOME%\node_modules\n8n" set "NEEDS_INSTALL=1"

if "%NEEDS_INSTALL%"=="1" call :install_n8n
if "%INSTALL_FAILED%"=="1" (
  echo.
  echo [WARN] npm install failed - trying npx fallback...
  echo.
  set "USE_NPX=1"
  goto :start_n8n
)

if not exist "%N8N_BIN%" (
  echo.
  echo [WARN] n8n binary not at expected path, using npx fallback.
  set "USE_NPX=1"
)

:start_n8n
echo.
echo [INFO] Starting n8n...
if defined N8N_EDITOR_BASE_URL echo [INFO] URL    : %N8N_EDITOR_BASE_URL%
echo [INFO] URL    : http://localhost:%N8N_PORT%
echo [INFO] Sign in with the owner account you created on first launch.
echo [INFO] Close this window to stop n8n
echo.

cd /d "%N8N_HOME%"

if defined USE_NPX (
  echo [INFO] Launching via: npx n8n@%N8N_VERSION% start
  "%NODE_EXE%" "%NODE_DIR%node_modules\npm\bin\npx-cli.js" n8n@%N8N_VERSION% start
  if errorlevel 1 (
    "%NODE_DIR%npx.cmd" --yes n8n@%N8N_VERSION% start
  )
) else (
  call "%N8N_BIN%"
)
pause
exit /b 0

rem --------------------------- subroutines -----------------------------------

:find_winget_node
rem Match any patch release rather than one hardcoded version.
if defined NODE_EXE goto :eof
for /d %%P in ("%LOCALAPPDATA%\Microsoft\WinGet\Packages\%~1") do (
  for /d %%Q in ("%%P\node-v*-win-x64") do (
    if exist "%%Q\node.exe" call :set_node "%%Q\node.exe"
  )
)
goto :eof

:set_node
if not defined NODE_EXE set "NODE_EXE=%~1"
goto :eof

:check_system_node
rem `node -v` prints e.g. v22.23.1, so compare the major-version prefix only.
for /f "tokens=*" %%V in ('node -v 2^>nul') do set "SYS_NODE=%%V"
if not defined SYS_NODE goto :eof
if "%SYS_NODE:~0,3%"=="v18" set "NODE_EXE=node"
if "%SYS_NODE:~0,3%"=="v20" set "NODE_EXE=node"
if "%SYS_NODE:~0,3%"=="v22" set "NODE_EXE=node"
goto :eof

:install_n8n
echo.
echo [INFO] Installing n8n v%N8N_VERSION%...
echo [INFO] This takes 5-15 minutes. Logs: %LOGFILE%
echo.

if exist "%N8N_HOME%\node_modules" (
  echo [INFO] Removing old node_modules...
  rmdir /s /q "%N8N_HOME%\node_modules"
)
if exist "%N8N_HOME%\package-lock.json" del /f /q "%N8N_HOME%\package-lock.json"

pushd "%N8N_HOME%"
set "npm_config_ignore_scripts=true"
rem NOTE: the log path contains spaces, so the redirect target must be quoted.
call "%NPM_CMD%" install n8n@%N8N_VERSION% --legacy-peer-deps --no-audit --no-fund --ignore-scripts --loglevel=warn >> "%LOGFILE%" 2>&1
if errorlevel 1 set "INSTALL_FAILED=1"
popd
goto :eof
